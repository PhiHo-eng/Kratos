...
#include "adjoint_small_displacement_SIMP_element.h"
...
void AdjointSmallDisplacementSIMPElement<TPrimalElement>::CalculateStressDisplacementDerivative(const Variable<Vector>& rStressVariable,
                                                                                            Matrix& rOutput, const ProcessInfo& rCurrentProcessInfo)
{
    ...
    KRATOS_ERROR_IF(rStressVariable != STRESS_ON_GP)
        << "AdjointFiniteDifferencingBaseElement::CalculateStressDisplacementDerivative: Invalid stress variable! Stress variable not supported for this element!" << std::endl;

    TracedStressType traced_stress_type = static_cast<TracedStressType>(this->GetValue(TRACED_STRESS_TYPE));
    KRATOS_ERROR_IF(traced_stress_type != TracedStressType::VON_MISES_STRESS)
        << "AdjointFiniteDifferencingBaseElement::CalculateStressDisplacementDerivative: Invalid stress type! Stress type not supported for this element!" << std::endl;
    ...
    KRATOS_ERROR_IF(dimension != 3)
        << "AdjointFiniteDifferencingBaseElement::CalculateStressDisplacementDerivative: Invalid element dimension! Currently only 3D SmallDisplacementElements are supported!" << std::endl;
...
template <class TPrimalElement>
int AdjointSmallDisplacementSIMPElement<TPrimalElement>::Check(const ProcessInfo& rCurrentProcessInfo) const
{
    KRATOS_TRY;
...
template <class TPrimalElement>
void AdjointSmallDisplacementSIMPElement<TPrimalElement>::save(Serializer& rSerializer) const
{
    KRATOS_SERIALIZE_SAVE_BASE_CLASS(rSerializer, BaseType);
}

template <class TPrimalElement>
void AdjointSmallDisplacementSIMPElement<TPrimalElement>::load(Serializer& rSerializer)
{
    KRATOS_SERIALIZE_LOAD_BASE_CLASS( rSerializer, BaseType);
}

template class AdjointSmallDisplacementSIMPElement<SmallDisplacement>;
